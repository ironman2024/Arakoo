...
const { ipcRenderer } = window.require ? window.require('electron') : {
    ipcRenderer: {
        invoke: () => Promise.resolve({ success: false, error: 'Electron not available' }),
        on: () => {},
        send: () => {}
    }
};
    ...
    try {
        if (isMacOS) {
            ...
            if (mediaStream.getAudioTracks().length > 0) {
                ...
                console.warn('No audio track available in media stream');
            }
...
// Create reference to the main app element - will be set by React app
let cheatingDaddyApp = null;
...
const cheddar = {
    ...
    getCurrentView: () => cheatingDaddyApp?.getCurrentView?.() || 'main',
    getLayoutMode: () => cheatingDaddyApp?.getLayoutMode?.() || 'normal',
